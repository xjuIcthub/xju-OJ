...
| GET | `/api/admin/export_problem` | 管理端；问题/竞赛 owner | Query 重复 `problem_id=1&problem_id=2`；普通 GET | ZIP 二进制，含 problem.json、测试数据和历史 AC 代码 | `ExportProblemAPI` / `ExportProblemRequestSerialzier` / 无专门测试 |
| POST | `/api/admin/import_problem` | 管理端页面；CSRF exempt；View 本身未显式权限装饰器 | multipart：`file`；QDUOJ ZIP：`N/problem.json`、`N/testcase/...` | `{"import_count": n}` | `ImportProblemAPI` / `UploadProblemForm,ImportProblemSerializer` / 无专门测试 |
