# Step 13：Backend Python 3.12 基础镜像

## 目标

在 uv 安装器稳定后，评估并切换 backend 的容器基础镜像，仍固定 Python 3.12；本 Step 不升级 Django、Redis、数据库驱动或 Worker。

## 进入条件

- Step 12 的 locked uv 镜像通过。
- Step 02 已有 Alpine cold/warm 构建、镜像大小和 native wheel 指标。
- Python3.12 micro、OpenSSL、libpq、Pillow 等 native 依赖可在候选镜像中验证。

## 选择门

优先候选：

- `python:3.12-slim-bookworm`，具体 patch + digest。
- 若组织要求容器也 Ubuntu，则另做 Ubuntu24.04 等价镜像；不能把宿主约束误当成容器约束。
...
docker buildx build --file backend/Dockerfile \
  --build-arg BASE_IMAGE=python:3.12-slim-bookworm@sha256:<verified> \
  --tag xju-oj/backend-py312-slim:<git-sha> .
docker run --rm xju-oj/backend-py312-slim:<git-sha> \
...
Compose/CI 恢复上一 Python3.12 基础镜像 digest；不回退 pyproject/lock 的业务升级。
...
```text
build(backend): pin Python 3.12 runtime base image
```
