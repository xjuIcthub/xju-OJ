...
14. 每个 build/compose/smoke 步骤的 stdout/stderr 必须实时显示在终端，同时完整写入本次 `runtime/deployments/history/attempt-*` 日志；无输出超过 10 秒时显示等待心跳；通过独立状态文件保留原始退出码，不能因 `tee` 丢失失败状态。
