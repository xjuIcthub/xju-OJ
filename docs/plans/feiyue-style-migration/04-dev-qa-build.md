...
- 开发 fixtures：后端无种子数据时 `/`、`/problem`、`/contest` 和首页分别显示 3 道示例题（整数/双精度 A+B、Special Judge A+B）与 2 场各含两题的 Upcoming Contests；真实 API 有数据时自动替换 fixtures。截图记录：`/tmp/xju-rank-table-mock.png`、`/tmp/xju-home-mock.png`、`/tmp/xju-problem-mock.png`、`/tmp/xju-contest-mock.png`。
- 当前开发环境已额外写入可复现的运行时演示数据：公开题目 `1001`（整数 A+B）、`1002`（双精度 A+B）、`1003`（Special Judge A+B）均含完整描述、输入/输出说明、样例和测试点；Special Judge C++ checker 已通过 `SPJCompiler.compile_spj()` 编译检查。比赛 `101` 索引 `1001/1002`，比赛 `102` 索引 `1002/1003`，比赛详情的 Problems 区可跳转到对应 contest problem 路由。公共 API 返回无错误，`/contest/101/problem/1001/`、`/contest/101/problem/1002/`、`/contest/102/problem/1002/`、`/contest/102/problem/1003/` 均实际渲染对应题面、样例和提交区。
- 首页最终复核：左栏为 Upcoming Contests 与 Problems Set，右栏为较窄的 Notice Board 及 User Ranking；`Aug 29` 在 1360px 与 760px 视口计算样式中均为 `white-space: nowrap`。当前截图记录：`/tmp/xju-home-final.png`、`/tmp/xju-problem-1003-final.png`、`/tmp/xju-contest-101-final.png`。
